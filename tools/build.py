...
def build():
    ...
    bootstrap = (
        ...
        "  (function() {\n"
        "    var KEY = 'fhwa_tileHeight', DEFAULT = 220;\n"
        "    var clamp = function(v) { return Math.max(160, Math.min(320, v)); };\n"
        "    function applyTileH(v) {\n"
        "      v = clamp(parseInt(v, 10) || DEFAULT);\n"
        ...
        "      return v;\n"
        "    }\n"
        "    applyTileH(core.load(KEY, DEFAULT));\n"
        "    var slider = document.getElementById('settings-tile-height');\n"
        "    if (slider) {\n"
        "      slider.addEventListener('input', function() {\n"
        "        core.save(KEY, applyTileH(slider.value));\n"
        "      });\n"
        "    }\n"
