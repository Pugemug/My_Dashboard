...
def build():
    ...
    bootstrap = (
        ...
        "  (function() {\n"
        "    var TILE_ORDER_KEY     = 'fhwa_tile_order';\n"
        "    var DEFAULT_TILE_ORDER = ['boxchart','saydoratioepics','happinessfaktor','flowefficiency','wip','akzeptanz'];\n"
        "    var canvas = document.getElementById('tile-canvas-lieferfahigkeit');\n"
        "\n"
        "    function getTileId(tile) { return tile.id.replace('tile-', ''); }\n"
        "\n"
        "    function applyTileOrder(order) {\n"
        "      if (!canvas) return;\n"
        "      var allIds = Array.from(canvas.querySelectorAll('.tile')).map(getTileId);\n"
        "      var full   = order.filter(function(id) { return allIds.indexOf(id) >= 0; });\n"
        "      allIds.forEach(function(id) { if (full.indexOf(id) < 0) full.push(id); });\n"
        "      full.forEach(function(id) {\n"
        "        var tile = canvas.querySelector('#tile-' + id);\n"
        "        if (tile) canvas.appendChild(tile);\n"
        "      });\n"
        "    }\n"
        "\n"
        "    function saveTileOrder() {\n"
        "      if (!canvas) return;\n"
        "      var order = Array.from(canvas.querySelectorAll('.tile')).map(getTileId);\n"
        "      core.save(TILE_ORDER_KEY, order);\n"
        "    }\n"
        "\n"
        "    function initTileDrag() {\n"
        "      if (!canvas) return;\n"
        "      canvas.querySelectorAll('.tile').forEach(function(t) { t.draggable = true; });\n"
        "      var draggedId = null;\n"
        "      var mousedownOnHeader = false;\n"
        "\n"
        "      canvas.addEventListener('mousedown', function(e) {\n"
        "        mousedownOnHeader = !!e.target.closest('.tile-header');\n"
        "      });\n"
        "\n"
        "      canvas.addEventListener('dragstart', function(e) {\n"
        "        var tile = e.target.closest('.tile'); if (!tile) return;\n"
        "        if (!mousedownOnHeader) { e.preventDefault(); return; }\n"
        "        mousedownOnHeader = false;\n"
        "        draggedId = getTileId(tile);\n"
        ...
        "        if (tile && getTileId(tile) !== draggedId) {\n"
        "          canvas.querySelectorAll('.tile').forEach(function(t) { t.classList.remove('tile-drag-over'); });\n"
        ...
        "        if (!targetTile || !draggedId || getTileId(targetTile) === draggedId) return;\n"
        "        var draggedTile = canvas.querySelector('#tile-' + draggedId);\n"
        "        if (!draggedTile) return;\n"
        "        var tiles = Array.from(canvas.querySelectorAll('.tile'));\n"
        "        var di = tiles.indexOf(draggedTile), ti = tiles.indexOf(targetTile);\n"
        "        if (di < ti) targetTile.after(draggedTile);\n"
        "        else         targetTile.before(draggedTile);\n"
        "        saveTileOrder();\n"
        "        canvas.querySelectorAll('.tile').forEach(function(t) { t.classList.remove('tile-drag-over'); });\n"
        "      });\n"
        "\n"
        "      canvas.addEventListener('dragend', function() {\n"
        "        canvas.querySelectorAll('.tile').forEach(function(t) {\n"
        ...
        "      var saved = core.load(TILE_ORDER_KEY, null);\n"
        "      applyTileOrder(saved || DEFAULT_TILE_ORDER);\n"
        "      initTileDrag();\n"
        "    });\n"
        "\n"
        "    var orderResetBtn = document.getElementById('settings-tile-order-reset');\n"
        "    if (orderResetBtn) {\n"
        "      orderResetBtn.addEventListener('click', function() {\n"
        "        core.save(TILE_ORDER_KEY, DEFAULT_TILE_ORDER.slice());\n"
        "        applyTileOrder(DEFAULT_TILE_ORDER.slice());\n"
        "      });\n"
        "    }\n"
        "\n"
        ...
        "        if (!window.confirm('Alle Einstellungen zur\\u00fccksetzen?\\n\\nDies l\\u00f6scht alle gespeicherten Konfigurationen und l\\u00e4dt die Seite neu.')) return;\n"
        "        var toDelete = [];\n"
        "        for (var i = 0; i < localStorage.length; i++) {\n"
        "          var k = localStorage.key(i);\n"
        "          if (k && k.startsWith('fhwa_')) toDelete.push(k);\n"
        "        }\n"
        ...
        "  // ── Tile-Vollbild-Modus ──────────────────────────────────────────\n"
        "  (function() {\n"
        "    var backdrop  = document.getElementById('tile-fullscreen-backdrop');\n"
        "    var panel     = document.getElementById('tile-fullscreen-panel');\n"
        "    var closeBtn  = document.getElementById('tile-fullscreen-close');\n"
        "    var titleEl   = document.getElementById('tile-fullscreen-title');\n"
        "    var fsContent = document.getElementById('tile-fullscreen-content');\n"
        "    var expandedTileEl   = null;\n"
        "    var originalParentEl = null;\n"
        "    var originalNextSib  = null;\n"
        "\n"
        "    function openFullscreen(tileEl) {\n"
        "      var titleSrc = tileEl.querySelector('.tile-title');\n"
        "      if (expandedTileEl && expandedTileEl !== tileEl) closeFullscreen();\n"
        "      expandedTileEl   = tileEl;\n"
        "      originalParentEl = tileEl.parentElement;\n"
        "      originalNextSib  = tileEl.nextSibling;\n"
        "      titleEl.innerHTML = titleSrc ? titleSrc.innerHTML : '';\n"
        "      fsContent.appendChild(tileEl);\n"
        "      panel.classList.add('open');\n"
        "      backdrop.classList.add('open');\n"
        "      document.querySelectorAll('body>div').forEach(function(el) {\n"
        "        if (el.style.position === 'fixed') {\n"
        "          var z = parseInt(el.style.zIndex, 10);\n"
        "          if (!isNaN(z) && z < 700) { el.dataset.prevZ = el.style.zIndex; el.style.zIndex = '700'; }\n"
        "        }\n"
        "      });\n"
        "      core.emit('resize');\n"
        "    }\n"
        ...
        "      if (!expandedTileEl) return;\n"
        "      document.querySelectorAll('body>div[data-prev-z]').forEach(function(el) {\n"
        "        el.style.zIndex = el.dataset.prevZ; delete el.dataset.prevZ;\n"
        "      });\n"
        "      if (originalNextSib) {\n"
        "        originalParentEl.insertBefore(expandedTileEl, originalNextSib);\n"
        "      } else {\n"
        "        originalParentEl.appendChild(expandedTileEl);\n"
        "      }\n"
        "      expandedTileEl   = null;\n"
        "      originalParentEl = null;\n"
        "      originalNextSib  = null;\n"
        "      panel.classList.remove('open');\n"
        "      backdrop.classList.remove('open');\n"
        "      core.emit('resize');\n"
        "    }\n"
        ...
        "    core.on('page', closeFullscreen);\n"
        "\n"
        "    requestAnimationFrame(function() {\n"
        "      document.querySelectorAll('.tile-expand-btn').forEach(function(btn) {\n"
        "        var tileEl = btn.closest('.tile');\n"
        "        if (tileEl) btn.addEventListener('click', function(e) {\n"
        "          e.stopPropagation();\n"
        "          openFullscreen(tileEl);\n"
        "        });\n"
        "      });\n"
        "    });\n"
        "  })();\n"
